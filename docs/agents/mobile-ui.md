...
- Mobile Agent management `Chat` actions should open a new-thread draft
...
- Mobile widgets are static snapshots: do not use `ScrollView`; start directly
  with thread rows, render pinned rows like other rows, and use agent
...
- Provider, agent, bot, and channel identity presentation must resolve
...
- Mobile low-frequency catalog data such as agents, workspaces, bots,
