...
- **搬迁(纯 Foundation,可整体进 Core)**:新建 `Sources/GaryxMobileCore/GaryxRelativeTimestamp.swift`,把
  ...
  (体内 `Date()` → `now`,其余逻辑不变)。Core 是 SwiftPM target,目录文件自动纳入,**无需改 pbxproj**。
- **调用方**:App 现有 9 处 `garyxFormattedTaskTimestamp(x)` + 2 处 `garyxThreadDate` 解析为 Core 版(走默认 `now=Date()`,**行为不变**);
  给缺 `import GaryxMobileCore` 的 App 文件补 import(多数已 import Core 类型)。App 文件**删除**这些函数定义(避免与 Core 同名歧义)。
...
- **pbxproj**:Core 文件 SwiftPM 自动纳入,**不改 pbxproj**。App-target 若新增文件才需 `xcodegen generate` + 提交 pbxproj。
  倾向 `GaryxRelativeTimestampText` 就近放 `SidebarViews.swift`(免 App 新增文件);如新增 App 文件则必须 xcodegen + 提交 pbxproj
  (否则 app 编译不到、`swift test` 假绿 —— 见 reference `iOS xcodegen .pbxproj 要手动同步`)。
...
2. **formatter 搬 Core**(`GaryxRelativeTimestamp.swift`,可注入 `now`)+ 删 App 拷贝 + 补 import + **Core 直测**(§4.2/§8)。
