...
### 3.2 方案:用 Core Animation 隐式动画替掉主线程 timeline(零 per-frame 主线程)
三点波改为「每点 opacity 在 `.repeatForever(autoreverses:true)` + 逐点 `.delay` 下脉动」,`.onAppear` 触发一次 `animating=true`:
```swift
struct GaryxAvatarTypingBadge: View {
    ...
    var body: some View {
        ...
        .onAppear { animating = !isPaused }
        .onChange(of: isPaused) { _, p in animating = !p }
        .accessibilityLabel("Running")
    }
}
```

### 3.3 为什么这才是真 O(1)(三选一论证,给 reviewer)
- **(A) 朴素「单 flag 全局翻一次」**:行不通——List cell 复用,后滚入的 badge 已见 `animating==true`,`.animation(value:)`
  不触发 → 晚到的 cell 永不动。**否决**(复用场景的硬伤)。
- **(B) 单主线程时钟(CADisplayLink/Timer)+ N badge 订阅**:tick 源收敛成 1 个,但每 tick 仍重渲染 N 个可见 badge 叶子
  → 仍是 O(可见 running) per-frame **主线程**工作,只是单元变小。比现状好,但没到 O(1) 主线程。
- **(C) 本方案 CA 隐式 `.repeatForever`**:动画交给 render server(CA)插值,**主线程 per-frame 工作 = 0**,且 cell 复用时
  `.onAppear` 重新起拍 → 晚到 cell 正确动起来。**真 O(1) 主线程,不随可见 running 行数放大。选 C。**
- 与 v4 §5「list 级单一 phase(O(1))」的关系:把「单一 30fps 主线程相位源」升级为「单一 CA 动画定义、render server 执行」,
  **更优**地满足"O(1) / 不随行数放大"。代价:各 badge 起拍时刻 = 各自 `onAppear`,**不严格同相**。
  行间距远、不会并列同框,同相与否肉眼无差(iMessage/Telegram 即如此)。**若 reviewer/老板要求严格同相**,降级到 (B):
  list 级单时钟 + badge 内部 `@ObservedObject` 订阅相位(只重渲 badge 叶子、不动行 body)。本稿默认 (C)。
...
### 4.2 方案:cell-local 每分钟自刷新叶子(不碰数据层 → 不撞 M5)
v4 §5 给了两条路:「数据层每分钟 tick 重投」**或**「cell-local 从 Date 算」。因 M6 与 M5 并行、**不得动数据层**,选 **cell-local**。
- 重构 `garyxFormattedTaskTimestamp` 加可选 `now` 形参(默认 `Date()`,**老调用全兼容**):
  `func garyxFormattedTaskTimestamp(_ value: String?, now: Date = Date()) -> String`(其余逻辑不变)。
- 新增叶子 `GaryxRelativeTimestampText`(home 专用,放 SidebarViews):
  ```swift
...
## 8. 测试与验证
- `swift test`(Core)全绿。M6 主要改 App-target 视图;Core 仅 `garyxFormattedTaskTimestamp` 加 `now` 形参——补一条
  「`now` 固定值 → 边界('now'/'59m'/'1h'/'23h'/'1d'/…)确定性」单测(去掉对真实墙钟的依赖,顺带固化不冻结逻辑)。
- `xcodebuild` Debug **SUCCEEDED**(App target,跑 xcodegen 同步 pbxproj 后)。
- 新增源文件(若 `GaryxRelativeTimestampText` 单列文件,或就近放 SidebarViews 内不新增文件)→ 若新增文件必须
  `xcodegen generate` 并提交 `.pbxproj`(否则 app 编译不到、swift test 假绿)。**倾向就近放 SidebarViews.swift 内,免新增文件。**
- packaged-app(`npm run dist:dir` 不适用 iOS;iOS 走 simulator 安装包)实测:swipe pin/unpin/archive、置顶重排、归档消失、
  下拉刷新、滚动跟手、running badge 动、timestamp 跨分钟变化、drawer 拖拽时行不可点 + 不滚。
...
2. **badge O(1)**:方案 C(CA 隐式)对比 v4 字面「单 phase」——是否接受"更优但不严格同相"?要严格同相则降级 (B)。
3. **timestamp 不冻结**:cell-local `TimelineView(.everyMinute)` 叶子是否够(只重渲叶子、不破复用)?`now` 形参重构是否够干净?
...
2. `garyxFormattedTaskTimestamp` 加 `now` 形参 + Core 单测。
3. `GaryxAvatarTypingBadge` 换 CA 动画。
4. `GaryxRelativeTimestampText` + `GaryxSidebarThreadRowView.liveTimestampValue` 接线。
...
7. `swift test` + `xcodebuild` + simulator 行为/视觉 parity + probe 对比。
