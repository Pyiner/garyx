...
日期：2026-07-16 · 状态：设计评审中（v11，回应 #TASK-2320 第十轮：统一 typed run admission + cron validation wire 合同）
...
- **bridge 非决策点原则（v9，取代 v6-v8 的发布协议）**：
  ...
  - **新增不变量（v11：统一 typed run admission，取代入口枚举）**：gateway/router 侧构造 `AgentRunRequest` 的唯一合法通道是两态 typed admission——
    - **`ThreadBound`**：构造时**必须持有真实存在的 canonical thread record 本身**（type-level 传 record 非 id 字符串——无法凭空构造；生产构造点 `chat.rs:376`、`cron.rs:2163`、`router/run/execution.rs:174` 全部改经此通道）。所有 existing 来源在 admission 处统一重验存在性：显式 `threadId` 不存在 → 404（修 `prepare.rs:473` 只查格式）；**bot endpoint 快照 stale（十轮反例：`routes.rs:566-572` 5s 快照→`prepare.rs:563-603` 不验存在→`delivery.rs:94-100` 空 callback→`run_management.rs:615` 首写 affinity）→ 绕过快照 fresh re-resolve，线程确已不在则按首次接触走 Fresh creator/gate，绝不携 `thread_data=None` 进 bridge**；`cron::` scheduled-reply 特判在 admission 下自然不可达（一并收口）；HTTP 与 WS 共用 prepare，同一合同。隐式创建唯一通道 = Fresh creator/gate；真实存在但无 stamp 的 legacy thread 走兼容 fallback（既有绑定，守恒）。
    - **`ProviderTool`**：基础设施工具（`tool_image.rs:186`、`commands/tool.rs:65`），**独立 typed 通道只允许 `tool::*` runtime id**（不允许 thread::/cron:: 混入），不创建 agent 绑定、不选择 agent（provider 显式固定），不受 agent enabled 管理——"CLI 不可派活"约束的是派给 *agent* 的工作。此定性可被用户否决；若否决则纳入独立 gate。
    - **thread-less cron 组合退役**：`AgentTurn` 无 workspace/thread/target、无 target 的 `SystemEvent` → load 标 invalid、派发 typed 拒绝（不进 bridge；validation wire 合同见 3.3）——不选"补建 canonical thread"，避免静默改变 legacy job 运行形态；用户显式改成合法组合即恢复（自然过 Fresh gate）。
...
- **invalid job 的 validation wire 合同（v11，回应十轮 #2）**：`CronJob` 运行时增加派生的 `validation_error: Option<...>`（load/merge 时计算，非持久化语义字段）；`AutomationSummary` 增加 `validation_state: "valid" | "invalid"` + `validation_error`（原因文案，如"缺 canonical target"）。**invalid 的 AgentTurn 必须仍进 `/api/automations`**：放宽 `is_automation_job`（`automation.rs:657-670`）让 thread-less AgentTurn 通过、summary 转换（`automation.rs:513-525`）对缺 workspace 产出 invalid 行而非整体失败——否则不可见不可修，"用户改成合法组合即恢复"无从谈起。无 target 的 `SystemEvent` 不是 automation（历史上也不在 automation UI），留在 `/api/cron/jobs` 并输出校验状态（`api.rs:1413-1430` 加字段）。desktop/iOS automation 列表渲染 invalid 行（错误 badge + 可进编辑修复）；CLI `automation list` 加 validation 列。**测试须含"存在 enabled agent"场景**：thread-less AgentTurn 仍被拒且原因 = 缺 canonical target，证明不是 disabled/全停用 gate 的假绿。
