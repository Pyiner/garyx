...
日期：2026-07-16 · 状态：设计评审中（v14，回应 #TASK-2320 第十三轮：入口穷尽收口 + 删除策略拍板 + persistence 两阶段语义）
...
- **bridge 非决策点原则（v9，取代 v6-v8 的发布协议）**：
  ...
  - **新增不变量（v12：sealed run admission + 原子 run intent，取代入口枚举）**：
    ...
    - **`ThreadBound` 的原子 admission = 校验 record + 取得 run lease（v13：完整 lease 生命周期，回应十二轮 #2）**：admission 在 thread store 的同一线性化点完成「record 存在且非 archived」校验并产生 **store-visible、per-request、cancel-safe 的 run lease**。生命周期合同：
      ...
      - **archive（`routes.rs:3108,3137-3186`）、独立 DELETE（`:3198-3259`）、底层 hard delete（`:1136-1173`）全部消费同一 lease 域**：delete/archive 先赢 → admission 返回 typed stale/not-found、bridge 零调用；lease 在持 → archive/delete 冲突拒绝（或显式中止），绝无"删除后再启动"。
    ...
    - **公开运行入口穷尽（v13/v14）**：`run_inline_streaming`（`run_management.rs:1203-1211`）、**`run_graph::execute_agent_run` 与 `RunGraphState`（`run_graph.rs:81-100,371-392`）一并收窄为 crate 内部**；**`ProviderRuntime::run_streaming`（`provider_trait.rs:107-126`）定性为有意保留的 provider SPI 例外**（数据面扩展点，供 provider 实现，收窄后仅 bridge 内部 run_graph 调用）——**不变量适用边界相应收窄为「garyx 生产 crate 可达的 run 启动入口」**，接口封闭性测试改为枚举 bridge crate 全部公开 API 中能启动 run 的函数（deny-list，防后加入口），并以生产调用树 grep 佐证 SPI 无旁路调用方。
    - **删除策略拍板（v14，回应十三轮 #2，不留"或"）**：**archive = 冲突拒绝**（活 lease 在持即 409，保持 `routes.rs:3137-3139` 现语义）；**DELETE = abort-and-drain 后删除**（保持"删除会停掉正跑 run"的现产品行为）：先进入 lease 域可见的 **`deleting` 状态**（新 admission 见之即返回 typed stale/not-found）→ 取消全部 request/active lease（cancel-safe 释放）→ **等待 run task 真正终止且 lease 归零**（现 `abort_run` 只 `task.abort()` 不等 JoinHandle `run_management.rs:1838-1847`，须补等待/释放确认）→ 执行删除。**lease 约束落在 `ThreadStore` 的 archive/delete 原语层**（单一咽喉），router 删除原语（`threads.rs:660-681`，cron 使用）与创建回滚删除（`routes.rs:2852,2865`）自动被覆盖——后者删除的是从未取得 lease 的新建线程，原语层无 lease 即放行，无需特判。
...
- **sealed admission 封闭性（v12/v13）**：编译期/接口测试——`AdmittedRun` 在生产模块外不可构造、**所有能启动 provider run 的公开方法**（含 `run_inline_streaming`）无裸生产入口；**`ProviderTool + metadata.agent_id` 反例**（被剥除/拒绝，不选 agent）；**TOCTOU barrier：`ThreadBound` 已构造后暂停 → 并发 archive/delete/hard-delete——两种先后序分别断言（delete 先赢→typed stale + bridge 零调用；lease 在持→archive/delete 冲突拒绝，绝无删除后启动）**。
- **lease 生命周期（v13/v14）**：`run_index` 已注册、transcript `run_start` 未提交时并发 archive → 被 lease 挡住；**persistence 首提交失败两阶段断言（v14 修正：首提交失败 ≠ run 结束）——①失败发生而 provider 仍被 barrier 阻塞：lease 在持、archive/delete 被阻；②provider 真正 terminal/error/cancel 后：lease 才释放**；普通 Started 成功 terminal 释放、provider error/panic terminal 释放；`QueuedToActiveRun` 成功 → 本请求 lease 释放无泄漏（既有 active lease 接管输入）；future cancellation/drop → 释放；同线程并发 follow-up → 独立 lease、排队语义不变；**DELETE abort-and-drain：deleting 态挡新 admission → 取消 lease → 等 task 终止 + lease 归零 → 删除（等待确认非仅 task.abort()）**；archive 对活 lease 409；创建回滚删除（无 lease）原语层直接放行。
