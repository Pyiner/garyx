...
日期：2026-07-16 · 状态：设计评审中（v13，回应 #TASK-2320 第十二轮：run lease 生命周期 + 公开入口穷尽）
...
- **bridge 非决策点原则（v9，取代 v6-v8 的发布协议）**：
  ...
  - **新增不变量（v12：sealed run admission + 原子 run intent，取代入口枚举）**：
    ...
    - **公开运行入口穷尽（v13，回应十二轮 #1）**：`run_inline_streaming`（`run_management.rs:1203-1211`，现仅测试调用）**收窄为测试/bridge 内部可见**，不改造为生产通道；编译期/接口封闭性测试覆盖**所有能启动 provider run 的公开方法**（`start_agent_run`、`dispatch`、`run_inline_streaming` 及任何后加入口），不止两个。
...
- **lease 生命周期（v13）**：`run_index` 已注册、transcript `run_start` 未提交时并发 archive → 被 lease 挡住；persistence 首提交失败 → lease 正确释放；`QueuedToActiveRun` 成功 → 本请求 lease 释放无泄漏（且既有 active lease 接管输入）；future cancellation/drop → 释放；同线程并发 follow-up → 独立 lease、排队语义不变。
