...
日期：2026-07-16 · 状态：设计评审中（v10，回应 #TASK-2320 第九轮：封死三条直达 bridge 旁路，canonical 绑定不变量）
...
- plugin account None→claude 的 bridge fallback 实际在 `lifecycle.rs:190`（`:168` 是 API account 注释）。
...
- **bridge 非决策点原则（v9，取代 v6-v8 的发布协议）**：
  ...
  - **新增不变量（v10）：agent run 只能携带真实存在的 canonical thread 绑定启动**。九轮发现的三条直达 bridge 旁路逐条封死/定性：
    1. `/api/chat/start` 的显式 `threadId` **必须真实存在，否则 404**（gateway 边界校验，修 `prepare.rs:473` 只查格式的现状；隐式创建唯一通道 = Fresh creator/gate）。真实存在但无 stamp 的 legacy thread 走兼容 fallback（既有绑定，守恒）。
    2. **`cron::<job_id>` 伪 thread 直发分支退役**：任何将启动 agent run 而无 canonical thread 的 cron 组合（`AgentTurn` 无 workspace/thread/target、无 target 的 `SystemEvent`）**load 时标记 invalid（可见错误态，automation list 展示）、派发时拒绝（typed 错误，不进 bridge）**——不选"补建 canonical thread"路线，避免本特性静默改变 legacy job 的运行形态；用户要恢复这类 job 须显式改成合法组合（届时自然过 Fresh gate）。
    3. **provider 工具（`tool::image::*` 等）定性为基础设施例外**：不创建 agent 绑定、不选择 agent（provider 显式固定），不受 agent enabled 管理——"CLI 不可派活"约束的是派给 *agent* 的工作。此定性可被用户否决；若否决则纳入独立 gate。
...
- **旁路封死（v10）**：**全停用 + 不存在的显式 `threadId` → 404 且 bridge 零调用**；真实 legacy 无 stamp thread → 兼容 fallback 照常续跑；thread-less cron 组合（config 与磁盘两来源 × AgentTurn/SystemEvent × disabled/全停用）→ load 标 invalid + 派发 typed 拒绝 + automation list 可见错误态、bridge 零调用；`garyx tool image` / `/api/tools/image` 在全停用下照常工作（基础设施例外定性）。
