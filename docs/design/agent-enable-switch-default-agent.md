...
日期：2026-07-16 · 状态：设计评审中（v7，回应 #TASK-2320 第六轮 3 条阻断；前五轮 26 条已关闭）
...
- **bridge 一致性合同（v7：双计数器 + staging 边界，回应六轮 #2）**：
  - **两个版本域分开**：`agent_state_revision`（仅 agent store mutation 时锁内递增）只用于**拒绝旧 agent snapshot**；另设 **`reconcile_generation`**（**每次** coherent reconcile 发布时递增，无论触发源是 boot、custom-agent mutation、settings reload、`garyx.json` 文件热更还是 MCP config 变更）——bridge 按 `reconcile_generation` 排序**整个 unit**。纯 config 热更（agent revision 不变）因 generation 递增而正常应用，不会被丢弃。
  - 所有刷新入口（boot、agent CRUD/toggle/default、settings reload `api.rs:2017`、文件热更 `app_state.rs:283`、CLI reload `gateway.rs:232`、MCP config）**全部收敛进同一串行 reconcile 通道**，把 `(AgentAvailabilitySnapshot, 派生的 provider topology/routes)` 作为一个 coherent unit 构造并一次发布。
  - **staging/swap 边界**：unit 在通道内旁置构造（staged），构造期间**不得原地修改共享 provider 默认值**（`get_or_create_provider` `lifecycle.rs:323` 的 pre-publish 原地写改为 staged 构造）；发布 = 原子 swap，读取方要么全旧要么全新。
...
- **target→generated 模式转换 = 新绑定**（统一判据）：服务端在 update 落库前对转换后将用于新线程的 agent **强制 enabled gate**；**current 必须实时读取目标线程当时的绑定**（不得用启动时缓存——线程可在运行期经 task assign 获得/变更绑定，`gateway/tasks.rs:825`）；目标缺失或无绑定时**要求显式选择 enabled agent**（否则 400 明确报错引导重选）；disabled → 400。generated 内只改名（无模式/agent 变化）仍走 current 档放行。
...
- **bridge 一致性**：默认热切换后 plugin None 路由同步；**barrier 乱序推送（旧 generation 后到被丢弃）**；**同 agent_state_revision 下纯 config C1→C2 正常应用（generation 递增）**；**config/agent 交错乱序**；**半应用窗口不可见（staged 构造 + 原子 swap，构造期无共享默认值原地写）**。
