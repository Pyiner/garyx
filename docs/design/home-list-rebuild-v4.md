...
- **§3.3 三源折叠(修订4,codex MAJOR4,Gary 原写错)**：**不用 max-seq-wins**——三源无统一可比 seq(`runTracker` 状态机无 transcript seq `ConversationRunTracker.swift:78`、committed 有 seq、recent 只有 projection)。改 **source precedence**:本地乐观 `runTracker.isThreadBusy` > 网关流 committed `runStateByThread.busy`(带 seq) > `recent_threads` projection;跨源固定优先级、不比 seq,每源内部用自己 epoch/新鲜度。反例守:local busy 后旧 recent idle 晚到,**precedence 保证 idle 不覆盖 busy**(max-seq 会错覆盖)。
