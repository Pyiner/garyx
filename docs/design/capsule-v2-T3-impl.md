...
- 入参 `capsule: GaryxCapsuleSummary`（可为合成的 deleted summary）。本地 `@State phase: {loading,
  ...
  `await model.loadCapsulePreviewHTML(capsule.id, capsule.revision)` → set phase（**不依赖
  `model.capsules` 查找**，删后/合成也能 fetch→404→deleted）。
...
## 8. 客户端 update/delete 取数语义（doc §6，B1）
...
func loadCapsulePreviewHTML(capsuleId, revision, forceRefresh: Bool = false) async -> GaryxCapsulePreviewHTMLResult {
    ...
    if !forceRefresh, let cached = capsuleHTMLCache[key] { return .html(cached) }
    ...
    catch let e as GaryxGatewayError { if case .httpStatus(404, _) = e { return .deleted }; return .failed }
...
- delete：marker 仍在 → 卡仍派生，serve 404 → disabled "Capsule deleted"。即时性靠重连/重开/下一帧
  （doc §6 首版可接受；DELETE handler 推帧是 gateway，越界不做）。
...
`GaryxMobileModel+Capsules.swift`（loadCapsulePreviewHTML/presentConversationCapsulePreview；删
