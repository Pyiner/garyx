...
- 卡 `GaryxCapsuleGalleryCard`：上半 `GaryxCapsulePreviewThumbnail`（§4，clipped 圆角 14 + hairline，
  aspect≈16:10）；下半 title 单行 ellipsis（空→"Untitled Capsule"）+ 1 行 small metadata（updated
...
struct GaryxCapsulePreviewThumbnail: View {
    ...
    //   else phase=.loading; await model.loadCapsulePreviewHTML(capsuleId,revision) → set phase
}
```

- `isActive` 来源：画廊 = `coordinator.activeIds.contains(id)`；聊天卡 = `activeKeys.contains(实例key)`
  （§7 线程下传）。false → 不进 webview 分支（离屏卸载，doc §7）；再 active 时 phase 多已 `.loaded`
  → 立即复挂、不重 fetch。
...
`@Published var conversationCapsulePreview: GaryxCapsuleSummary?`（会话 cover）。两者皆
...
- `GaryxMobileCapsuleChatCardsView`（`GaryxMobileCapsuleViews.swift`）：纵向 stack（通常 1 张，
  ...
  \(card.capsuleId)")`）、下半 title（空→"Untitled Capsule"）+ `GaryxCapsuleChatCardPresentation.
  subtitle`。tap → `Task { await model.openMobileRoute(.capsule(card.capsuleId),
  source:.conversation) }`。缩略图 `.deleted` → disabled "Capsule deleted"。
...
1. **缓存 prune 接到 `capsules` 更新单点**：`@Published var capsules`（主文件 `GaryxMobileModel.swift`）加
   `didSet { pruneCapsuleHTMLCache(validCapsules: capsules) }`（`pruneCapsuleHTMLCache` 现为
   `+Capsules.swift` 的 `private`，须改 internal 供跨文件 didSet 调用），覆盖
   **所有**更新路径（中心 catalog `:602`、gallery `refreshCapsules`、`deleteCapsule`、gateway reset）。
   任一 capsules 列表更新即驱逐**已删 capsule**（id 不在新列表）的 `(id,*)` 缓存 → 下次缩略图取数
   cache-miss → `/serve` → 404 → "deleted"。`refreshCapsules`/`deleteCapsule` 里原显式 prune/filter
   变冗余可删。**list 缺席只作缓存失效提示，非删除判定**（删除判定恒是 `/serve` 404 vs 200）→ 刚建但
   列表暂未含的 capsule 至多多 fetch 一次（200 重缓存），**绝不误标 deleted**。
...
- 综合：focused 即时权威；缩略图经任一 capsules 刷新（中心/画廊/会话 route-time）失效 → 至多陈旧到
  「下次刷新/重开」（parent §6『重开后更新』可接受），**不会会话内永久陈旧**。DELETE handler 推帧是
  gateway，越界不做。
...
`capsules` 加 `didSet { pruneCapsuleHTMLCache(validCapsules: capsules) }` 单点失效，§8.1 防线 1，
自动覆盖中心 catalog `+Gateway.swift:602`）、
