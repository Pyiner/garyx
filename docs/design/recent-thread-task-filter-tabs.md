...
- v3（2026-07-12）：吸收 #TASK-2154 设计 review 首轮意见。明确 Mac 从
  `/api/threads` 换到 canonical `/api/recent-threads` 后，显式
  `exclude_from_recent` / automation generated threads 退出 Recent 是有意产品变化；
  补 iOS `sidebarThreadSummary(for:)` 消费者和辅助 All / near-end / scope / mapper
  测试约束。待复评。
...
|---|---|---|---:|---|
| `All` | 全部 | `tasks=include` | 是 | canonical Recent 中的普通 thread + task backing thread |
| `Chats` | 对话 | `tasks=exclude` | 否 | 排除 `thread_type="task"` |

产品要求：

1. 冷启动、Gateway 切换后默认 `All`；iOS 保持现有 canonical Recent 成员集，Mac
   则有意从 `/api/threads` 全量列表切到 canonical `/api/recent-threads` 成员集：
   显式 `exclude_from_recent` 和 automation `generated_thread` 不再出现在 Recent；
...
- 不从 `DesktopState.threads` 删除 automation generated thread；Automation 页面、
  thread direct open 和其它全量消费者继续可达。
...
换源有一个已确认、且有意采纳的成员集差异：`/api/threads` 默认只排除
`hidden`，而 `recent_threads` 投影还排除 `exclude_from_recent=true` 和
`automation_thread_mode="generated_thread"`。因此当前会出现在 Mac rail 的
automation generated thread 将退出新 Recent；这使 Mac 与 iOS/bot 的 canonical
Recent 语义一致。该 thread 仍保留在 `DesktopState.threads`，Automation 页面可用
`automation.threadId/targetThreadId` 打开，不构成归档或数据丢失。side chat 同时带
`hidden=true`，旧/新 Recent 都不可见，是成员集 parity 的控制样本。
...
这份 Recent 顺序。另一个读点是
`GaryxMobileModel+Presentation.swift::sidebarThreadSummary(for:)`，它在全量 `threads`
查找之后又查 `recentThreads`；因为 `recentThreads` 本来就由 `threads` 按 ids 投影，
该分支是冗余 summary lookup，不应随 filter 获得可见性语义。
...
offset 也做有限非负整数校验。scope 归一化复用 main
`garyx-client/http.ts::normalizeGatewayUrl` 的单一实现；renderer 使用 main 返回/
DesktopState 已归一化的 scope，不另写一份 URL normalizer。page size 固定为 100，
Gateway 上限 200 不变。

`/api/recent-threads` row 使用 `title`（不是 `/api/threads` 常见的 `label`），没有
`created_at/worktree`，但提供 `last_active_at/run_state` 并由 route 附
`thread_runtime`。`fetchRecentThreads` 复用/扩展 `mapThreadSummary` 的兼容字段 mapping，
不得假设 `/api/threads` 独有字段存在。
...
`allRecentThreads` 或 `visibleRecentThreads`。`sidebarThreadSummary(for:)` 是顺序无关
的 summary cache lookup：保留其 `selectedThread` → `threads` → Pinned fallback，删除
冗余的 `recentThreads` 分支，不绑定主页 filter。
...
对已经使用 `/api/recent-threads` 的 iOS，默认 `All` 在旧/新 Gateway 上保持当前
成员集。Mac 是本期第一次从 `/api/threads` 切到该 canonical projection，明确接受
§3.2 所述 `exclude_from_recent` / automation generated-thread 退出 rail 的一次性成员
集变化；这不是旧 Gateway fallback，也不能用客户端 union 补回（union 会破坏
filter 域的 `total/has_more/offset`）。`DesktopState.threads` 和 Automation 入口不变。
...
- source-membership regression：automation generated thread 保留在
  `DesktopState.threads` 且 Automation `Thread` action 仍可达，但不进入两个 Recent
  feed；hidden side chat 在旧/新 Recent 都不可见；
- component：tab accessible name/selected state/左右键、空态、cached refresh、footer、
  `onNearListEnd` 只在阈值内触发且重复触发由 controller coalesce。
...
- selected=`nonTask` 时辅助 All head refresh 会启动并 coalesce，结果只写 All，失败
  不污染 selected presentation，且 pull-to-refresh completion 不等待它；
- Pinned task 在 `nonTask` 下仍进入 Pinned section；
- Widget snapshot 与 Automation options 始终读取 `All`；
- `sidebarThreadSummary(for:)` 继续从共享 summary cache/Pinned 解析 Automation/Bot
  entry，不随 visible filter 改变；
...
过滤”的错误；另外包含一个 automation generated thread 和一个 hidden side chat，
用于证明 canonical Recent 与全量 thread cache 的成员集边界：

1. 首次进入 Mac/iOS：默认 `All`，task 可见；automation generated/hidden side chat
   不在 Recent，但 generated thread 仍留在 Mac `DesktopState.threads` 且可从
   Automation 的 `Thread` action 打开；
...
| §3.2 Mac 现状 | PASS-WITH-DESIGN-FIX | `AppShell.tsx::recentThreadRows` 直接 map `desktopState.threads`；`threads.ts::fetchThreads` 默认 `/api/threads?limit=1000`；`store.ts::getDesktopStateFast` 使用同 route 的 200 行 fast page；`recent_thread_projection.rs::is_recent_thread_excluded` 额外排除 explicit recent exclusions | v2 漏了两个 route 的成员集谓词差异。v3 明确采纳 canonical Recent：automation generated threads 退出 rail，但保留在 DesktopState/Automation；核心结论（不能改 DesktopState.threads 语义）不变。 |
| §3.3 iOS 现状 | PASS-WITH-DOC-FIX | `GaryxGatewayClient.listRecentThreads` 只发 limit/offset；`GaryxMobileModel` 的 `recentThreadIds` + `threadListPager`；`HomeProjectionActor`、`persistRecentThreadsWidgetSnapshot`、Automation 三处 `model.recentThreads`；`sidebarThreadSummary(for:)` 有一个冗余 lookup | Widget/Automation 读取 All；Automation 仍把 recent 与 cached threads 去重合并。v3 补齐第五个读点并把它定性为共享 summary lookup，删除冗余 recent branch。 |
...
### 13.2 #TASK-2154 设计 review 首轮反馈处置

- **F1 接受并修订**：评审证实 `/api/threads` 仅按 `hidden` 排除，而
  `/api/recent-threads` 还排除 `exclude_from_recent` / automation
  `generated_thread`。在“Mac 必须显式发送 `tasks=` 且服务端过滤后分页”“不做客户
  端 union fallback”“原则上不扩 Gateway 生产 API”三项约束下，采纳 canonical
  Recent 是唯一不破坏分页契约的方案。产品取舍记录在 §1/§3.2/§10，回归与 E2E
  记录在 §11.2/§11.4。
- **F2 接受并修订**：补 `sidebarThreadSummary(for:)`，明确使用共享 cache 而不是
  visible feed，见 §3.3/§7.2/§11.3。
- **非阻断建议全部吸收**：辅助 All completion boundary、near-end、单一 URL scope
  normalizer、recent row mapping 均已进入 §6/§11。
