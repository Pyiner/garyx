# Garyx 桌面横向布局状态机 + 面板扩窗 — 综合设计裁决(final v6)

日期:2026-07-13(v6:thread logs 移入 right-docked SideTools，不再参与横向布局)
...
- 单一事件 `LAYOUT_INTENT_CHANGED { previousOccupancy, nextOccupancy, cause, transactionId }`,一次携带三面板完整 desired occupancy(全向量)。
...
- **冷启动(fresh session)**:新增 `CLAIM_INITIAL_LAYOUT` command——仅 fresh session 可用、不改 bounds;renderer 按写死顺序 sidebar → rail → sideTools 提交初始可见 track 的认领**与初始 desiredOccupancy(参与双 revision CAS)**,main 原子建立 `fundingByPanel + desiredOccupancy` 与 `normalBaseBounds = 当前宽 − Σ认领`。由此启动即展开的 sidebar(245)是 funded:首次显式关闭会正确缩窗(否则违反 §0)。
...
| 9 | minWidth / 主区 | **480**;主区硬保护 **350**;测试区分 outer bounds / content bounds / primaryThreadWidth |
...
| 12 | thread logs | 移出 horizontal layout，替换 right-docked SideTools 中原有的 Tasks tool；日志只作为 SideTools 内容并继承其开合、resize 与 responsive 行为，无独立 occupancy/funding/dock/overlay/resizer/width preference。全局 Tasks 入口与 `#/tasks` 路由、thread task tree 保留 |
...
| 19 | 不变量 | **v6 改:替换为 §3 八条** |
| 20 | 持久化 | v1 裁定:**现状即契约**——持久化仅 `garyx.sidebarCollapsed`(localStorage);sidebar/rail/sideTools 宽度 session-only;新增宽度持久化 → P2;responsive/临时态永不写偏好 |
...
## 3. 不变量(v6,替换 I1–I5;全部表驱动 headless)
...
3. responsive presentation 改变不修改 requested intent;只有偏好 commit 写 persistence。
4. 仅 user/display-origin 的权威宽度 snapshot 更新 responsive basis。
5. 每个 bounds command 持有合法 `BoundsAuthority`:UserCauseToken(仅用户动作铸造)或 RepayProof(只能减少已确认 funding,绝不扩窗);snapshot-only transition 不能铸造 UserCauseToken,可延续/终止既有 authority。
6. `confirmedFunding + normalBaseBounds` = 最近一次 acknowledged session;intent closed 而 funding 暂存的合法状态仅两种:带 authority 的 deferred reconcile,或 hydrate 发现的 orphanedFunding(必须立即 RepayProof 偿还)。
7. 无外部几何变化且 transaction 全 settled 的限定下,funded open→close 恢复原 bounds。
8. 所有 accepted 物理 result 按 window revision 单调折叠;旧 sequence 不得抹掉更新 revision 的物理事实。
...
- **Phase 0b 事件归一化(UI 不变)**:三面板全部 writer 汇入一条 occupancy event log(仍走旧 controller),证明 route/capsule/cleanup 无漏口。
...
- projection:§3 八不变量;宽度档 480/720/721/960/961/980/981/1116/1280/1480/1920 × 面板组合 × 窗口模式;capacity 降级链。
