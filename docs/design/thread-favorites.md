...
Status: draft v16 (addressing review round 15 — 2×P1, no new P0)
Date: 2026-07-16
...
- **pre-R5 升级 cutover 屏障（round15-2）**：新进程取锁后、destructive
  init 之前，若 PPID 指向仍存活的同名 garyx 进程（fallback spawn 亲缘），
  有界等待其退出（cap 60s）再继续；托管路径天然序列化无窗口；非托管手工
  升级须先停 pre-R5 进程（发布文档写明；非亲缘旧进程无法从新二进制侧探测，
  边界如实声明）。
...
### 4.2 `/api/recent-threads`（All/Chats）——CONFIRMED

cursor-only；行级 `activity_seq`；响应带身份二元组；bot reader 内部 offset。
...
- **响应判定三步（round14-4，一切 favorites/feed 响应与 flight 结算共用）**：
  ...
  3. 当前 epoch：身份 unknown → bootstrap acceptance；
     ...
     新基线；意图不跨域保留）。
- **mutation readiness（round15-1）**：favorites 写与 archive/delete 派发
  前置要求当前 epoch 身份已 known；unknown → 先触发一次身份 bootstrap
  （拉 snapshot/recent，有界等待），失败按今日网络失败 UX 报错、不派发。
  入口不做常驻禁用（点击时惰性 bootstrap）。
- `server_boot_id` 失配（同 incarnation）= feed 整替提示（不清意图）。

### 7.2 意图 reducer——CONFIRMED（含 wrong_incarnation → 按 §7.1 三步走）。
...
- **cutover 屏障（round15-2）**：无锁旧父进程存活 → 新二进制启动 →
  destructive init 延迟至父退出（或 cap）后才执行的确定性用例。
