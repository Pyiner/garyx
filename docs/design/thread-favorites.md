...
Status: draft v24 (addressing review round 23 — final wording fix)
Date: 2026-07-16

## 0a. Round 22 findings → v23 处置

| # | Finding | 处置 |
|---|---|---|
| 1 | P1 「生命周期双端调用完全不动」与 R1「一切写单 attempt」矛盾（现码 archive/delete 走 idempotent 双 attempt，实测 attemptCount==2）——保 parity 违反 R1 且继续掩盖首次 ambiguous，落实 R1 必然改变 transport 行为 | **parity 边界精确化**：lifecycle 的「今日 parity」限定为**服务端路由、请求体、无身份先验、业务幂等语义不变**；双端 transport 照 R1 **机械迁移为 single-attempt**（archive/delete 同其它写），ambiguous 按保守分类回喂编排层 → 触发 forceReplacement（§4.4 既有行为）。测试补 archive/delete `attemptCount == 1`（§2、§4.4、§5、§8） |
| 2 | P2 标题仍 v21 | 已更正 |
| 3 | P2 §4.0 泛称「mutation 必带 expected_store_incarnation」 | 改为「favorites mutation」 |
| 4 | P2 §4.2 recent 响应二元组/三元组表述不一致 | 明确：recent 响应带 `{store_incarnation_id, server_boot_id}` 二元组；仅 favorites 页与 snapshot 另带 `revision` |
...
**承重事实（round 21 P1-2 修正后的如实表述）**：
1. **服务端 CAS 双围栏（`expected_revision` + `expected_store_
   incarnation`）封死的是「携带旧凭据的写」**——旧 incarnation/旧
   revision 的请求（含孤儿/迟到/跨恢复）必 409 零副作用。CAS **不证明
   目标 provenance**：unknown → bootstrap 拿到新库凭据后，排队意图 drain
   到恢复库中同 ID 实体上的「陈旧目标重绑定」是**显式接受的残余风险**
   （§4.6-1：良性布尔、可见、一键可逆、管理事件级频率）。客户端
   admission/provenance 是针对该残余的不承重保险带，v21 移除。
2. 「同 URL 换库」是**重启级管理事件**（data_dir 启动时读取）。**新建**
   线程 ID 为 uuid（threads.rs:123）；但合法性检查仅要求 `thread::` 前缀
   （thread_logs.rs:110），**legacy import 保留原样 key 不受 uuid 约束**
   ——同 ID 碰撞的现实来源是同源备份/恢复库系，其 incarnation 轮换
   （§3.3）保证旧凭据写被拒。
...
**系统级连带改造（用户裁决授权，改根因并说清楚；范围受「不要无限制扩大」
裁决约束）**：R1 传输契约（+auth middleware tagged，仅 iOS/desktop）、
R2 组合快照端点、R3 keyset 分页 + activity_seq、R5 data-dir 独占锁 +
store incarnation（**仅收藏 CAS 域**，含启动装配重构）。
（生命周期幂等化与全写面身份加固 = 「store 身份与生命周期写安全加固」
独立后续任务。）
...
`serverIdempotencyKey`；**生命周期端点的服务端改动**（路由/请求体/无身份
先验/业务幂等语义全部今日 parity；根治入后续加固任务）。注意：双端
transport 按 R1 机械迁移单 attempt **覆盖 archive/delete**（round22-1，
非服务端改动）；收藏侧唯一编排行为 = ambiguous 后触发 forceReplacement。
...
**favorites mutation** 必带 `expected_store_incarnation`（先验
incarnation 后验 revision）。
...
- `/api/recent-threads`（All/Chats）：cursor-only；行级 `activity_seq`；
  bot reader 内部 offset。——CONFIRMED
- **（v23 表述统一）身份字段仅覆盖收藏域承重响应**：
  - recent-threads 响应带 **`{store_incarnation_id, server_boot_id}`
    二元组**（无 favorites revision）；
  - favorites 三端点页与 snapshot 带
    **`{store_incarnation_id, server_boot_id, revision}` 三元组**。
  其余线程/实体响应不加身份包络（provenance 机制随 v21 裁决移除；全实体
  身份包络入后续加固任务）。
...
### 4.4 生命周期操作（v23：parity 边界精确化）

- **服务端今日 parity**：archive/delete 的路由、请求体、无身份先验、业务
  幂等语义完全不动（生命周期身份围栏与幂等化在「store 身份与生命周期写
  安全加固」后续任务）。
- **双端 transport 按 R1 迁移（round22-1）**：archive/delete 与其它写
  一样 `mutationSingleAttempt`（现码 idempotent 双 attempt 掩盖首次
  ambiguous 的行为废止）；ambiguous 按保守分类回喂编排层。
- 收藏侧唯一编排行为：ambiguous 结局在既有报错/回滚之后触发
  **forceReplacement**（All/Chats 整替 + favorites snapshot）；承诺边界
  「快照前提交 → 行消失；之后 → 下周期收敛」。
...
## 6. Desktop / iOS 落点——同 v14，v21 收束

- archive/delete 的**路由、请求体、调用签名及身份参数不变**；transport
  按 R1 迁移为 single-attempt（§4.4/§5），ambiguous 回喂编排层后触发
  forceReplacement。
- favorites 写的 wrong_incarnation 409 → 按 §7.1 三步判定处理。
...
- **favorites 写派发前置（v21 收束）**：= 当前 epoch known(uuid) 且已
  ...
  未就绪时 toggle 照常入 reducer（意图排队，presented 即时生效）；
  **就绪来源 = 周期 snapshot/GET**（首刷失败 → 下一周期重试，意图持续
  排队，诚实降级），就绪后 drain 派发。无 admission gate / provenance
  前置：stale 凭据由服务端 CAS 409 兜底，客户端按三步判定与 conflict
  分支收敛；换库瞬间误收藏为显式接受残余（§4.6）。archive/delete 沿
  今日路径（无收藏侧前置）。
...
- favorites：CAS/守卫/同快照组页/孤儿写/删除交错/无条件 bump；
  **incarnation**：wrong_incarnation 409 零副作用（**仅 favorites 路由**，
  v21 裁决）；recovery 旋转后旧 expected 不可提交。
...
**传输契约（R1）**：同 v14；另补 **archive/delete `attemptCount == 1`**
（round22-1：替换现有 idempotent 双 attempt 断言测试，如
testGatewayClientRetriesNetworkConnectionLostOnPost 对应场景改为单
attempt + ambiguous 分类）。
...
- **身份初始化（round15-1，v21 保留）**：cold start unknown → 首个身份
  ...
  就绪时排队、就绪后 drain；**首刷失败回归**（首个 snapshot/GET 失败 →
  意图持续排队 → 下一周期成功后 drain）；`GET /api/store-identity` 契约。
...
5. **双端状态机与 feed**：reducer + 三步判定 + range-fill（K 窗口 + head
   复核）+ snapshot 原子接受 + lifecycle ambiguous 后 forceReplacement
   （先测后 UI）。
6. **UI**：入口与 tab + xcodegen。

**独立后续任务（已立项，更名）**：「store 身份与生命周期写安全加固」——
承接 v21 移出的全部项（生命周期幂等化 / operation token / delete
tombstone / 清理 outbox / 生命周期身份先验 / 统一写围栏 middleware /
WS 握手身份 / MCP path context / history GET 去写 / 全实体身份包络 /
跨语言 manifest / catalog 缓存身份 / CLI 身份迁移）。
