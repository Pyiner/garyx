...
Status: draft v20 (addressing review round 19 — 2×P0 + 2×P1)
Date: 2026-07-16
...
### 4.0b 统一 store 写先验（v20 重写：按 mutation-capable transport 建模）

- **HTTP store 写**（POST/PUT/PATCH/DELETE 中触及 thread store 的路由）：
  统一 middleware/提取器在 handler 副作用前校验
  `X-Garyx-Expected-Store-Incarnation` 头；失配 → 409 tagged
  `wrong_incarnation`（零副作用）；缺失 → 400 tagged。
- **WS（round19-1①）**：`/api/chat/ws` 升级请求必带
  `expected_store_incarnation` query，**升级前校验**，失配拒绝升级——
  升级后 socket 上的 start/input/interrupt 写继承握手时绑定的身份；
  重启/换域后的延迟握手被拒。
- **读路径去写（round19-1②）**：`GET /api/threads/history` 的
  `set_logged`（api.rs:1055）从读路径**删除**（违反 repository-contracts
  「读路由不做修复」），迁至显式写路径（实现时定位其唯一消费者并给出
  等价 POST 或写路径搭车方案）。
- **MCP（round19-1③）**：gateway 为 provider 会话构造 MCP endpoint URL
  时把 incarnation token **编入 path context**（Claude 剥自定义
  header/query 不影响 path），进 rmcp 前校验；schedule_followup /
  capsule_create/update 等写工具由此获得围栏。
- **豁免清单**（= 客户端 control 语义面，§5）：auth、restart、health、
  settings/provider 配置等非 store 写；source-guard 测试钉住豁免面 +
  **GET upgrade / GET 内写 / nest_service 一并扫描**。
- favorites 与 archive/delete 的显式身份参数并入统一头；favorites 的
  `expected_revision` 仍为端点级 CAS 参数。
- `GET /api/store-identity` → `{store_incarnation_id, server_boot_id}`
  （CLI/简单客户端 bootstrap）。

### 4.1 收藏读写（CAS）——CONFIRMED（身份改走 §4.0b 头）

`PUT/DELETE …?expected_revision=N`（+ 统一身份头）；200 恒 bump 含
no-op；409（revision/incarnation 分 code）/404 tagged + 同快照页 + 身份；
400 tagged。
...
- **（round17-2 + round18-2）身份包络统一形状**：一切产出线程实体的响应
  顶层带一次身份——单实体 `{store_incarnation_id, server_boot_id,
  thread}`、集合 `{…, threads: […]}`、**嵌套实体同规**（如
  `/api/automations/{id}/threads`，automation.rs:1097/1177）。覆盖清单：
  `/api/threads`（desktop 全量 slice）、`/api/threads/{id}`（双端点查：
  pinned 窗口外修补 store.ts:1085、iOS +ThreadList:50/+StateSync:79/
  getThread）、线程创建响应、**PATCH 返回 summary**（iOS 整对象覆盖路径
  +ThreadLifecycle:345 原子替换 summary+provenance）、automation 嵌套。
  ingest 规则 = 响应先过 §7.1 三步判定，再**原子**给所含全部 summary 盖
  provenance。**producer source-guard**：扫描返回 thread summary 的
  route handler，allow-list 强制身份包络。
...
**（v20 重写）传输语义三分**：
- `readRetryable`：无副作用读，可自动重试，不附身份。
- **`storeBoundMutationSingleAttempt(frozenIdentity)`**：一切 store 写。
  **只接受调用方传入的冻结身份**（round19-2）——effect/IPC 携带
  `(scope, epoch, authorizedIncarnation, actionToken)`，transport 附
  frozen 值并在派发前复核当前状态 == 冻结值（不等 → actionCancelled），
  **绝不替换为自己的 current identity**；调用方无冻结身份（= admission
  未过）→ 拒发。desktop IPC 契约（rename/archive/delete/favorites 等
  输入）加身份元组，main 校验并透传（round19-2 的 IPC 洞封闭）。
- **`controlMutationSingleAttempt`**：控制面写（settings、provider auth、
  restart、config 等），不附身份、不受 unknown 限制。**豁免清单与
  §4.0b 服务端 middleware 豁免一一对应**（同一份清单，双侧共享 + 双侧
  测试）（round19-3）。
...
- **mutation readiness（round15-1 + round16 修订）**：
  ...
  - unknown → 点击时惰性 bootstrap（有界等待），失败按今日网络失败 UX
    报错、不派发；入口不常驻禁用。
  - **admission 序列（round16-1 + round18-3）**：admission 状态机唯一
    owner = Core/ingress（宿主 fire-and-forget 只提交点击事件）。点击 =
    **冻结 desired** + `admissionGeneration[threadId] += 1`，token =
    `(gatewayScope, runtimeEpoch, threadId, admissionGen)`；**同线程新
    点击立即使旧 admission token 失效**（异线程互不取消）；bootstrap
    await 返回后仅 token 仍 current 才可调 `toggle`（scope/epoch 变化 →
    永久取消，新域身份不得唤醒旧操作；admission 逆序完成 → 最后点击胜）。
  - **目标 provenance（round16-1 + round18-2 措辞更正）**：provenance =
    **任何经身份包络 ingest 的实体**（不限 feed 行；含点查、PATCH 回写、
    automation 嵌套行等）；mutation 仅允许目标 provenance == 当前
    known(uuid)；provenance unknown（持久化 fallback 缓存行）或失配 →
    不派发，触发刷新，用户在新行上重新确认。bootstrap 只建立读基线，
    不授权来源身份未知的目标。
...
- **round17-2 入口矩阵**：pinned 窗口外行（点查修补）、深链、workspace/
  bot 钻取、新建线程、**automation 嵌套行、PATCH 回写行**——各入口行均带
  provenance，favorites/archive 入场校验可判定（不再有永拒或全局补标
  路径）。
- **round18-1/round19-1 mutation-capable transport**：store 写路由缺失/
  失配身份头 → 400/409 零副作用（PATCH thread 等穷举写面）；**WS 握手无/
  错身份 → 拒绝升级**（延迟握手跨重启被拒）；**history GET 无写副作用**
  （set_logged 已迁出，读路径零写断言）；**MCP path context 身份校验**
  （错 token 进不了 rmcp，capsule/schedule_followup 写被围栏）；豁免清单
  source-guard（含 GET upgrade / GET 内写 / nest_service 扫描）；
  `GET /api/store-identity` 契约。
- **round19-2 冻结身份交接**：effect 携带 frozen 身份、transport 附
  frozen 值且派发前复核（切域后 → actionCancelled、零派发）；desktop
  IPC 身份元组透传校验；**round19-3** control 面调用（settings/restart）
  在 unknown 身份下正常可用；两侧豁免清单一致性测试。
- **round18-3 admission 序列**：同 ID 两次反向点击 + admission 逆序完成 →
  最后点击胜（B 侧观察终态 = 第二次点击 desired）；异 ID 并发 admission
  不互相取消（双端）。
...
## 9a. CLI 迁移面（v20，round19-4）

- CLI gateway helpers：启动 `GET /api/store-identity` 取身份；store 写按
  `storeBoundMutationSingleAttempt` 附冻结头 + 恰一次 attempt（现
  `ConnectOnly` 重试语义仅保留给读）；覆盖 thread/task/agent/automation
  写与 `garyx message`（/api/send，message.rs:48）。
- **包络解码适配**：`thread get/create` 等把响应当裸 summary 的解码器
  （thread.rs:44/66）与打印器（thread.rs:3 顶层读 `thread_id/label`）
  适配 `{store_incarnation_id, …, thread}` 单实体包络；全 CLI 解码点
  grep 清单入实现 PR。
- CLI 的 chat/ws 使用（thread.rs:172）随 §4.0b 握手带身份。
- 测试：CLI 写面清单（无身份 → 本地拒发而非服务端 400）、包络解码
  round-trip、人类输出字段不回退为 `-`。

## 9. 实现切分（六步，同仓同发）

0. **gateway-identity-transport**：store 写先验 middleware + WS 握手绑定 +
   history GET 去写 + MCP path context + 豁免清单/source-guard +
   store-identity 端点（§4.0b）。
...
3. **gateway-recent**：activity_seq 全套 + seq keyset + 行级 seq + boot_id
   + source-guard。
4. **传输契约**：iOS + desktop（含 web-api.ts）+ **CLI（§9a）**；三分
   语义 + 冻结身份交接。
