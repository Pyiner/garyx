...
Status: draft v19 (addressing review round 18 — 1×P0 + 2×P1)
Date: 2026-07-16
...
### 4.0b 统一 store 写先验 middleware（v19，round18-1）

- 一切 **store-bound 公开 HTTP mutation**（POST/PUT/PATCH/DELETE）经统一
  middleware/提取器在 handler 副作用前校验
  `X-Garyx-Expected-Store-Incarnation` 头；失配 → 409 tagged
  `wrong_incarnation`（零副作用）；缺失 → 400 tagged。
- **豁免清单显式枚举**（auth、restart、health、非 store 配置写等），
  source-guard 测试钉住豁免面（新增 store 写路由默认落入先验）。
- favorites 与 archive/delete 的 `expected_store_incarnation` 参数并入该
  头（单机制）；favorites 的 `expected_revision` 仍为端点级 CAS 参数。
- 新增 `GET /api/store-identity` → `{store_incarnation_id, server_boot_id}`
  （CLI/简单客户端 bootstrap 用）。
...
**（round18-1）transport 中央身份挂点**：mutation 请求由 transport 层自动
附 `X-Garyx-Expected-Store-Incarnation`（当前 known(uuid)）；unknown →
transport 拒发并触发身份 bootstrap（readiness 中央强制）；readRetryable
读请求不附。
...
- **round17-2 入口矩阵**：pinned 窗口外行（点查修补）、深链、workspace/
  ...
  路径）。
- **round18-1 middleware**：store 写路由缺失/失配身份头 → 400/409 零副作
  用（PATCH thread 等穷举写面）；豁免清单 source-guard；transport 中央挂
  头（unknown 拒发触发 bootstrap）；`GET /api/store-identity` 契约。
