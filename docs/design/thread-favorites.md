...
`serverIdempotencyKey`；**生命周期端点的任何改动**（无身份先验、无幂等化
——完全今日 parity，唯一收藏侧行为 = ambiguous 后触发 forceReplacement；
根治入后续加固任务）。
...
### 4.4 生命周期操作（v21 裁决：完全今日 parity）

- **archive/delete 的服务端契约与双端调用完全不动**：无身份参数、无先验、
  无幂等化（round14-3/round18-1 的生命周期身份围栏随 v21 整体移入
  「store 身份与生命周期写安全加固」后续任务）。
- 本设计唯一的收藏侧行为：客户端 ambiguous 结局在既有报错/回滚之后触发
