...
Status: draft v15 (addressing review round 14 — 2×P0 + 3×P1)
Date: 2026-07-16
...
- **响应判定三步（round14-4，一切 favorites/feed 响应与 flight 结算共用）**：
  ...
  2. **旧 epoch → 直接丢弃**（不检查 incarnation，防旧页触发二次清场
     切回旧域）；
  3. 仅**当前 epoch** 响应的 `store_incarnation_id ≠ 当前已知` → 真实
     换域 → scope-clear（epoch bump、废弃 flight/effect、清 raw/水位/
     fence/意图/feed、重拉 snapshot 建新基线；意图不跨域保留）。
