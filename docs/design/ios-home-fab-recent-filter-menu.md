...
- **图标** `line.3.horizontal.decrease`:SF Symbols 标准筛选语义,裸 glyph
  ...
  `*.circle` 变体(圆中圆)。
- **iOS 26 玻璃两条硬规则**:glass 直接挂在 label 内容上(非
  `.background {}`,避免被容器 hoist 盖住前景);显式
  `.contentShape(Circle())` 补 hit-test。与现有顶栏按钮写法一致。
- **回调链路不变**:`onSelect` 仍指向
...
1. **iOS 26 Menu 玻璃 label 的 hit-test**:现有顶栏玻璃按钮 +
   ...
   `allowsHitTesting(false)` 背景装饰 + contentShape 承接点击」写法。
