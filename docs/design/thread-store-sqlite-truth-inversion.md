...
```
调用方(router / bridge / gateway routes / tasks / automations)
...
| 低频 | create/label/binding/task CRUD | 整档案重写 |
...
exclude_from_recent/automation_*、
...
- **写**:走 GaryxDbService 现有的单 `Mutex<Connection>`,新增
  ...
  delta 级写频下会把其它写路径推进 SQLITE_BUSY
