...
(role/content/text/timestamp/metadata/agent 字段一致)——复印件与
transcript 尾窗在字节级同源,这是"换源即退役"的根本依据。
另:messages 条目里随 `runtime_context` 复印了 `garyx_mcp_auth_token`,
退役复印件同时消除这份令牌落盘拷贝(安全红利)。
...
且都消费"有界尾窗"——transcript 尾窗(有 tail cache,075be4dd 后为
流式)是逐字节等价的替代源;`message_count` 早已迁到
...
不重写业务语义。RecentThreadProjectingStore 的三件职责——per-thread
锁、archived 写拒绝、投影派生——整体折叠进 SqliteThreadStore,包装层
...
消费方(原生 provider 续聊、群转写快照)换读 transcript 尾窗 ≤100 条
`type=message` 记录——与今日快照逐字节同源(§3.2),`ProviderMessage::
from_value` 解析路径不变;预览/标签/标题类消费方改读写时维护的小字段
(`last_user_preview` / `last_assistant_preview` ≤160 字符,由 run 末尾
终局持久化写点维护)或任务自身字段。然后**停止写入**:快照重建、线程
导入播种的快照半边(transcript 半边保留)、任务体播种、trim、scrub 的
messages 合并、三处 `messages: []` 初始化全部移除,最后拆
`garyx-models` 类型化载体字段。收益立刻兑现(档案停涨、每次重写单位
缩到 sans-messages body、令牌拷贝不再落盘),且与 store 切换互相独立、
可分别回滚。导入迁移无条件剥离 `messages` 顶层键(含 `tool::image::*`
等非 thread 键——审计确认其 messages 同为化石);legacy 任务的标题
兜底由导入时把首条 user 消息一次性补进 `task.body`。
...
双目录解析 + scrub)→ 剥 `messages` → 单事务写入 record + 投影。
一次一条,峰值内存 = 单条记录(最大 8MB);1.2GB 流式读一遍,预估
秒级–几十秒,记录 `migration_states` 行(源文件数/导入数/跳过数)。
导入完成前不服务请求(避免半迁移脑裂;一次性成本可接受)。多机
(办公/家)各自首启自迁。

### D8 双写过渡与读切换(用户拍板路径的落地化)

config `sessions.thread_store`: `file` | `sqlite`(双写)| `sqlite-only`,
env `GARYX_THREAD_STORE` 可覆盖(免改配置急回滚)。
