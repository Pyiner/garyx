...
- Add only one provider-specific shared config field: `antigravity_bin`.
...
2. Scan `$HOME/.gemini/antigravity-cli/conversations/*.db` for files created or
   modified after run start.
3. Prefer the newest candidate whose matching brain log path exists.
4. Once found, emit `SessionBound { sdk_session_id: <id> }` and persist the id
...
Rows are processed once by `step_index`; if a row is rewritten while being
tailed, the later complete parse wins before emission.
...
- Config defaults and builder precedence for model, binary path, workspace,
  env, and timeout.
- Command construction for first run, continuation, and print timeout.
- Transcript parser:
