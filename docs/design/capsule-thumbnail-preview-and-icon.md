...
| --- | --- | --- |
| headless Chromium（chromiumoxide/CDP，或 shell 系统 Chrome） | ✅ | 浏览器 ~150–300MB；**绝不能嵌进 release 二进制**（撞体积门，与"release 不嵌 Bun"同因）；要么强依赖用户机装 Chrome（按 `GARYX_*` 解析、缺则报错，仿 workflow Bun 模式），对每台机是重依赖 |
...
**结论：gateway 侧预渲染是错的层。** 它要么嵌浏览器撞体积门（CLAUDE.md 明确"release 不嵌 Bun
以保体积门，勿再打包"——同一条红线），要么强依赖外部浏览器/破跨平台。收益（一张静态图）可以在
