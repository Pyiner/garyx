...
7. 增加 subprocess plugin capability detection。
8. 增加 legacy subprocess adapter，把 `StreamEvent` 降级成 `dispatch_outbound`。
9. 所有 run origin 切到共享 fanout，包括 channel-originated、scheduled、tool-triggered、restart recovery。
10. 删除 gateway-side message rendering、Markdown parsing、image stripping 和 channel-specific branching。
11. 所有受支持 plugin 都 stream-event native 后，删除 `TODO.md` 里的兼容债。
