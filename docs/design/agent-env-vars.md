...
- `updateAgent` 依 intent 应用：unchanged→沿用现有「回捞 baseAgent 保留」；replace/clear→发对应 map。
...
- `GaryxMobileModel+AgentsWorkspaces.swift` `updateAgent`（`:726-810`）：**编辑前确保取权威 env**
  （复用 / 扩展 `catalogSnapshotRestored` 回捞 `:748`，使编辑器 seed 的是权威 `providerEnv` 而非
  cache 剥过的投影），保存时按 intent 计算 `providerEnv` 请求值（unchanged→沿用 `baseAgent.providerEnv`
  即 `:779` 现状；replace→`Some(map)`；clear→`Some([:])`）；`createAgent`（`:681-724`）补发 env。
...
- **iOS**：`GaryxMobileCore` SwiftPM 测 env draft 序列化 / 校验 / intent 解析 / seed；**新增
  restored-cache 场景测试**（Blocker 3）——seed 的 agent `providerEnv` 被 cache 剥空、用户未改 env →
  保存必须解析出「保留权威 env」而非清空（对齐 `GaryxMobileCatalogCacheTests.swift:5` 的剥离前提）；
...
- **风险**：
  ...
  - iOS「编辑前取权威 env」若漏做 → 清空隐藏 env（Blocker 3）；由 §4.5 + restored-cache 测试守护。
  - native shell 注入是行为变化（此前只继承父环境）+ 回显泄漏（Q4）——测试锁定 + UX 提示 + 明确接受。
- **明确豁免**：内建 agent（架构边界）；`garyx.json agents.*.env`（既有、不在编辑面）；
  in-process native 的 LLM HTTP 调用（无子进程，env 走进程内凭证解析，已工作）；team 级 env。
