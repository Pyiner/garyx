# P0-A 设计 v42：交互式返回 + 手势物理统一

母计划：`mobile/garyx-mobile/docs/ios-fluid-interface-optimization-plan.md`。依据 Apple *Designing Fluid Interfaces*。**仅 iOS 26**。历史实施锚点曾以 `origin/main@03d999a38`（A1/A2）为准；A3-A5 已完成迁移，收官验收索引见 §8b。
...
**v42（TASK-2667）**：commit-settle 性能实测证明，release 同步发布 canonical/route-context/composer snapshot 并重配第一响应者会让 SwiftUI 与 UIKit 输入系统挤占移动帧。权威时序改为：canonical truth、data owner 与输入 reducer 在 release 立即提交；route/context/composer 的**可观察投影**及 focused adapter 的 `isEditable`/first-responder 收尾延至 terminal。settle 期间 source 焦点只作冻结的视觉连续性，不再接受输入。

**已定案项**（不再复述）：A0 结论与系统曲线；D2 物理层（**已合 main**：`03d999a38`，A1/A2 验收完成）；A3-A5 的路由、容器、耐久化、生产接线与手势迁移均已完成；home 呈现节点；UIKit 容器 VC + wrapper 变换契约；五维 owner 相位矩阵（§2.2）；touch-down 快照 edge 判定与仲裁表；drilldown predecessor；按钮同时序；部署目标 26（已合 main）。
...
| 刻 | 动作 |
|---|---|
| **release（commit 决策刻，MainActor 临界区）** | activation `live → finalizingInput`；冻结新用户输入；同步 `unmarkText()` + drain marked text；**提交 canonical path**（§4）。若 source 已聚焦，保留只读的 first-responder/键盘视觉连续性至 terminal；adapter delegate 已拒绝新输入，输入 reducer 与焦点视觉投影解耦 |
| **producer terminal（drain 完成刻）** | pending 异步 producer 的终结由 `InputFinalizationLease` 持有（不依赖 first responder 生命周期）。producer 集合空时分两况（r24-F1）：**reservation 边已 terminal**（none/committed/revoked）→ 进入下行「双边 terminal 事务」；**reservation 仍 sealed** → 只持久化 **`producerDrained(finalSequence, drainBuffer)`** 记录——**不物化 final、不建 N+1、不发 close** |
...
- **`ComposerPayloadStore`**：文本、附件、pending picker/upload operation 统一入店；主键 = 不可变 **`ComposerPayloadEntryID`**；**Entry 内子项有独立身份（r28-F2）**：`attachmentID`/`operationID` 为子项键——operation terminal（failedTerminal/cancelled）**只删除该 O 的 `stagedAssetID` + operation record**，同 Entry 的文本与其他附件不动；**EntryID 仅在内容为空且无 delivery/feedback/alias 引用时回收**（feedback 引用在场则 Entry 保留，反馈永有匹配对象）。测试：文本+两附件其一失败/取消，其余内容与反馈可达性不变。（threadID/draftID/alias 只是索引）；**按 GatewayScope 分区**；**废除旧全局附件数组**。payload 是**用户数据**（与 RouteInstanceStateStore 缓存分离、不入 LRU）：raw binary 落受保护磁盘暂存（排除备份、复制前原子配额预留，内存只留缩略图/路径/metadata）；operation record terminal 前 pinned；send/reset/移除/scope revoke 确定性清理本地文件；配额超限 = 用户可见失败。**文件清理登记是独立 durable condemned tombstone**：reset/移除事务可原子吊销 operation、释放 owner/配额并移除 manifest，但必须保留该登记跨 relaunch；只有物理文件已幂等删除后的独立确认事务才可移除登记，禁止以 domain reset 代替物理删除确认。**per-key 附件随 key 保留**（旧"切 key 清空全局数组"行为废除——按新架构语义）。
...
- **process-death manifest + 逐状态恢复表（r19-F7/r23-F2）**：无密 `{OperationCapabilityKey, staged path, state, uploadAttempted}` 持久化。重启结算：`requested/preparing`（picker 已消失）→ 原子 cancel + 清理 staging；`uploading 且未 attempted` → scope/auth 匹配可安全重试（请求未发出过）；**`uploading 且 attempted` → 不自动续跑**（gateway 每请求生成新 UUID 路径，续跑必产生第二份远端文件）——转 failed-retryable 由用户显式重附；gateway 幂等上传（P0-G）落地后才开放 attempted 自动续跑。测试：上传请求发送前/服务端写入后响应前/ack 后逐点 kill；连续两次 kill/relaunch（failedRetryable 不衰变）；无 operation 的 seal→输入→release→drained commit→kill 序列（G+2 + close exactly-once）；**gateway switch/logout × failedTerminal × relaunch**（suspended 反馈保留于 origin 分区且不进入 composer 呈现、revoked 零 UI）。
...
- **A4d-1 L-1 concrete choice（钉死）**：swap 提交事务保留 O1 的 `superseded` 审计记录，但在同一事务内清空 `O1.stagedAssetID` 与 `O1.reservedBytes`，文件、配额与 manifest owner 仅归 O2；不采用“保留带 assetID 的 O1”或另事务清理形态。
- **`PayloadConflictSet` = durable candidate list（r19-F6）**：冲突载荷是**用户数据**——每个候选持稳定 EntryID 入持久化列表，由 payload 配额计量，**永不静默丢弃**；**既有 EntryID 的 candidate membership 元数据成本固定预留**（配额只挡新增二进制）；**准入 fail-closed（r21-F3）**：candidate 准入与 domain promotion 在**同一 durable 事务**——准入无法 durable 提交（ENOSPC/IO/fsync）则 **promotion 整体不提交**，source draft/key 保持权威（升格稍后重试），不存在"promotion 成功但载荷无主"的中间态；failpoint 测试覆盖 candidate insert/pin/promotion 各边界的 ENOSPC/fsync/crash + relaunch 后可达性。该列表只承担内部 durable ownership，不再派生 payload-conflict 卡片、用户 action 或 pending-decision 交互态；恢复载荷由 producer 按 §1.6 自动安置。测试：3+ 并发冲突、kill/relaunch 列表存续、quota-full promotion × session drain × relaunch 后可达性。
...
- 测试：file/photo/camera 延迟完成 × pop/deep-link/promotion/gateway-switch × 双 host 全组合（双 fake gateway 记录 request body，G1 文件零命中 G2）；beginSend×pending×success/failure/cancel；session 退休后 completion 落对条目；空/非空 host 自动恢复分支 + 多 deferred payload 顺序顶入 + 重启存续；10 图/多文件/跨 20 route/suspend/revoke 的 RSS+磁盘清理+恢复。

- **durable feedback 状态机（r28-F3，权威正文）**：`pending(scope, EntryID, operationID, kind) → presented → acknowledged | archived`；`presented` 保留为持久化兼容状态，但 composer 不再调用 presentation planner，也不把任何 feedback 映射为内联文案或动作卡。**durable acknowledge 与领域动作事务绑定（r29-F3/r32-F3）**：`failedRetryable` 的重附 ack 并入成功 swap 事务；**`failedTerminal` 的重附 = 原子事务 `admitFreshOperation(O2) + acknowledgeFeedback`**（failedTerminal 事务保留**稳定 attachment slot/lineage tombstone 至 feedback 达 terminal（r33-F3：`acknowledged` 或 `archived` 均在同一事务释放 tombstone）**——O2 凭 lineage 落位原 slot，无需已删除的 oldKey）；移除的 ack 并入成功 cancel/delete 事务——动作事务失败（ENOSPC/fsync）则 feedback 保持 pending/presented，领域状态仍可恢复；仅"确认"（无副作用动作）可独立 durable ack。scope revoked → `archived` 零 UI；**identity 事件五分类（r32-F1/r34-F3）**：
...
- **A4d-1 有界历史钉死项**：PayloadGeneration 的永不复用证明由 `{generationClaimFloor, currentBlockExactClaims}` 表示；预提升下一 hi-lo block 的同一事务把 floor 推到旧 block 水位并清其精确集合，旧 block 尚未 claim 的 ticket 到此失效、调用方必须重新分配，故跨重启不复用且 claim 元数据恒受单 block（≤4096）约束。unsettled reservation ledger 限额为 per-scope 64 / global 256 / 4 MiB；terminal ledger 只在仍有 barrier/operation/manifest/replacement/producerDrained/recovered-close/delivery/discard-convergence 后代时保留，最后一个后代删除的同一事务回收，之后迟到事件按 unknown reservation 拒绝。identity discard 先把 barrier 归为 payload-free idle、再在同一事务删行。create-delivery 的非终态同样限 per-scope 64 / global 256 / 4 MiB；`acknowledged` 或 ambiguous 已获用户 disposition 后转 shared tombstone pool，超限先按 `(scope, createIntentID)` 回收 create correlation，再保持 `(ReservationID, DeliveryRecordID)` 顺序回收 terminal delivery；两类非终态与 `DiscardFinalizationTombstone` 永不静默淘汰。
...
| uploading × attempted | **不自动续跑**：转 failed-retryable、文件保留供重附、upload 不增；composer 不投影内联状态文案 | 同左（origin 分区） | 转 scopeRevoked、清 staging、留 correlation evidence |
...
| **failedRetryable**（含 attempted 转入） | **文件保留**供重试/重附；EntryID/G 不变；upload 不增；composer 不投影内联状态文案；**跨任意次 relaunch 保持本行**（不衰变为 terminal） | 同左（origin 分区） | 转 cancelled 行处理 |
| **failedTerminal**（不可恢复错误） | **单一 durable 事务（r30-F2）**：O 转 failedTerminal + 插入 `feedback.pending` + 建立 Entry feedback 引用 + 登记文件清理——物理删文件在提交后**幂等执行**（各写入边界 failpoint：不存在"附件消失且无 durable feedback"的中间态）；upload 不增，composer 不投影内联状态文案 | 反馈持久化于 origin 分区，不进入 composer 呈现 | 清理 + 诊断归档，**零用户 UI** |
...
（synthetic revoked 的 G+1 → G+2 durable 映射即 targetMapping；12 格输入 reducer 管文本、本矩阵管 operation，二者正交。）禁止无终局网络重试；放弃 provisional payload = 显式 cancel + 清文件 + durable feedback。**kill 语义拆分（r25-F1）**：`producerDrained` durable commit **前** kill → 窗口内缓冲 U 可丢（声明行为），重启回 G/T；commit **后** kill → U 已 durable，启动恢复**必**将其归入 G+2 并 close（不再宣称无条件丢失）；envelope 侧同理（outbox 未提交=无记录）。已消耗的 generation/ReservationID 空间作废不复用；**ledger 有记录者启动时已合成 revoked 终局**（按 12 格表处理），ledger 无记录的迟到旧 reservation 回调按 unknown reservation 拒绝并审计。**多发送并发（r20-F2）**：barrier 释放即可开下一次 beginSend；多个 `DeliveryRecord` 各自独立走 transport/reconcile/ambiguous（busy 续发/Queue-Steer 语义保留）——S1 ambiguous 不阻塞 S2。**durable 配额与背压（r21-F2）**：非 terminal DeliveryRecord 设 per-scope 与全局上限（**per-scope ≤ 64 条 / 全局 ≤ 256 条 / envelope bytes 计入 payload 磁盘配额**）；**seal 前预检**——超限时不 seal、草稿保留并写入 durable feedback，但 composer 不显示内联状态文案；**非 terminal 记录永不静默淘汰**（terminal 后转有界 correlation tombstone）。共享 tombstone 池超限的同一写事务按 `(ReservationID, DeliveryRecordID)` 升序确定性淘汰 terminal correlation 行直至数量与字节均合规；绝不淘汰非 terminal DeliveryRecord 或 `DiscardFinalizationTombstone`，若不可淘汰部分自身超限则事务 fail-closed。被淘汰 correlation 的迟到 evidence 按 unknown correlation 拒绝。测试：连续离线/ambiguous 发送到限、满池 discard、kill/relaunch、回收稳态。**乘积表**（SendCommitBarrier × InputFinalization，含**附件/picker mutation** 不只文本）：release 落窗口 → finalization 冻结 G+1 provisional 视角（envelope 不属 N final）；suspend/revoke 于窗口 → 先结算 barrier（commit 或撤销）再结算 scope；同 key `inputReady` = finalization settled ∧ barrier idle。测试：全格 + S1 ambiguous×S2 acknowledged + 连续两次 sealed + 乱序 frame + revoke + fsync 失败后迟到双代回调 + relaunch。fsync 不上 MainActor。
- transport 尝试前原子推进 `transportAttempted`；`notDispatched`（持久化证明未越传输边界）= 唯一"安全重试"，但**不静默重发网络请求**：裸消息在启动恢复或 live attempt-marker 提交失败后，以单一事务终结原记录为 `abandoned/restoredToDraft`、移除 host delivery 引用并释放 64/256 live 配额，同时自动安置 immutable envelope。host 仅含空白且无附件/operation 时原位采用；host 有任意有效输入时保持其最新意图不变，recovered 作为独立 durable payload 排队，在当前 draft 已提交到 durable delivery pipeline 后按 recovery generation 顺序自动顶入，或在后续 activation/relaunch 发现 host 为空时顶入。全程不派生二选一卡片或用户 action。未完成 multi-stage create 所拥有的记录不走此路，保留 create 不明态领域状态。A4d-1 legacy 附件记录若只有 attachment IDs 而无 snapshot，至少恢复文本并持久化"部分附件未恢复，请重新附加"feedback，绝不因无法重建旧附件而悬挂；该 feedback 不进入 composer 呈现。
- **ambiguous 领域出口保留、composer 零呈现**：恢复为草稿 / 重发副本的原子事务与 `DeliveryEvidence × UserDisposition` 正交模型保持不变，但任何 `transportAttempted`、`ambiguous`、失败或恢复中间态都不映射为输入框内联文案或动作卡。迟到 server frame 只推进 evidence（自动收口为 acknowledged）；终结记录清 payload 只留有界 correlation tombstone。
- **kill/relaunch 矩阵**：outbox commit 前后/attempted 后 transport 前/transport 后 response 前/ack 后逐点杀进程重启，终态 ∈ {acknowledged, 已回收为可操作草稿的安全重试, 用户可终结}，永不悬挂；crash-before-transport dispatch=0 且出口可用；自动回收出口自身每个 SQLite 写边界再 kill→relaunch，断言空 host 已原位恢复、非空 host 与 deferred recovered 均逐字节保全、notice/action 为空、delivery reference 与 live 配额均归零、二次恢复幂等。
- **新建会话多段写（r19-F2 如实建模）**：create thread → optional bind → chat start 每段持久化 CAS 状态。**消息级认领成立**（chat-start 的 `origin_id` 链路实测存在）；**create 级认领当前 gateway 不可证**（`/api/threads` 不回 metadata、无 (scope,createIntentID) 唯一索引）——**create-response 丢失如实建模为 ambiguous**，恢复为草稿 / 可能产生重复 thread 的重建事务保留，但不进入 composer 呈现；**不承诺"无幽灵/重复 thread"**；该保证的启用条件 = P0-G 的 `(scope, createIntentID)` 唯一索引 + 查询接口（已列 P0-G 前置项）。
...
| commit-settle | **destination**（释放刻原子写） | **destination**（可预载） | **冻结** | source 残留冻结（已聚焦键盘保持静态，输入已拒绝） | 无（新手势不 eligible） |
...
| focus/键盘 | ✓ | ✓（focus continuity 归 source） | ✓（仅冻结的视觉连续性；terminal 收尾） | ✗（§4） | ✗ |
...
- **commit 权威步骤 = §1.4a release 刻**：冻结输入 reducer + 同步 unmark/drain + 原子写 path **同一 MainActor 临界区完成**；不发布 SwiftUI composer snapshot。已聚焦 source adapter 保持 `isEditable`/first-responder 仅用于静态键盘连续性，delegate 拒绝新输入；terminal 再一次性置只读并 resign。close 于**双边 terminal 事务**发出（producer drained ∧ reservation 结算，§1.4a）；ack 由 capability 独立保证。**不存在"先 close 再写 path"的旧时序**。
...
3. **XCUITest**：finish/cancel/regrab/深栈/首层；deep link 排队；键盘三元组 + CJK finish（真 UIKit host marked range 组合态）+ 听写 pending 两分支；升格并发输入；同 key occurrence（A→P→A、A→A、深链）；RTL 实坐标；边缘两用例；横滚共存；`.payloadPreparing` 锁、ambiguous 出口、自动 recovered 安置、配额失败 + 各面 VoiceOver。
4. **性能/内存**：§3.2 全门 + alias 不变量（drain 后 =0、bytes ≤64KB）+ finalizer 入预算 + payload 磁盘配额清理。
5. **持久化 harness（真实 kill→relaunch）**：§1.6 发送矩阵逐边界（含 `commitSend → transportAttempted` 裸消息自动回收事务的全部 SQLite 边界、空/非空 host 自动安置、deferred payload 存续、零 recovery notice/action 与配额归零断言）；多段 create 每段"服务端提交后客户端确认前"死亡；**sealed 窗口 operation manifest × reservation outcome 恢复矩阵**；**全 operation state × destination 删除 × kill/relaunch 资源归零矩阵**；**discarding 收敛全边界真实 kill（r36-F3/r37）**：进入 discarding/强制 revoke/producerDrained 合成/closePendingAck 转 tombstone/**逐 DeliveryRecord CAS**/资源清理/token discarded 每个持久化边界杀进程重启续收敛（含无 operation 路径、**barrier idle+多 record 混合三相位+transport 竞争**、**S₁ pendingAck+S₂ live 与多 pending ack 乱序 ack×crash**、**D:S₁ closePendingAck→promote D→T→T:S₂ live/finalizing→discard T 或 scope revoke（逐 session tombstone 写入间 crash，断言两 session 均结算、token discarded、alias 回收、零 pinned finalizer/host）**、500 次多 session discard churn）；operation manifest 恢复/取消；ambiguous 出口自身 crash 原子性；出口 × 迟到 frame 竞争；durable 三合一 fsync 断言；真 `SQLITE_FULL` 回滚；deferred recovery 跨重启存续并按序顶入；scope 化 composer 重启恢复。
...
| **A5** | 抽屉/task tree/row swipe 迁移 | 旧 drawer opening/closing gesture 及返回分支、旧 app 层 sidebar axis 判定（Core 化后删）；`GaryxMobileTaskTreeSidebarViews` 旧 drag 状态机（原 `:188-294`）；`GaryxSwipeActionRow` 固定 spring settle 路径；旧取消自愈注释与 workaround（原 `GaryxMobileViews.swift:531` 等——新状态机显式建模取消后删）。验收脚本在 shell 中分段重组历史标识符，文档本身不得保留它们。 |
| **A6（收官切片）** | — | **全仓清理审计**：上述全部清单复扫归零；未被任何片消费的旧 motion token/辅助函数 dead-code 扫描（含低版本 `#available` 分支——政策仅 iOS 26）；xcodebuild warnings 零新增；设计文档内"现状锚点"段落改写为历史注记 |

每片 reviewer 必须把删除清单作为独立验收面（不删 = FAIL），并 grep 断言零残留。

## 8b. 验收记录索引

- A1/A2：`03d999a38`（D2 物理层与 Core 验收）。
- A3：`950f85a16`（路由/输入/耐久化 Core 收口）。
- A4a：[`ios-fluid-p0a-a4a-acceptance.md`](ios-fluid-p0a-a4a-acceptance.md)。
- A4d-1：[`ios-fluid-p0a-a4d1-acceptance.md`](ios-fluid-p0a-a4d1-acceptance.md)。
- A4b：[`ios-fluid-p0a-a4b-acceptance.md`](ios-fluid-p0a-a4b-acceptance.md)。
- A4c：[`ios-fluid-p0a-a4c-acceptance.md`](ios-fluid-p0a-a4c-acceptance.md)。
- A4d-2：[`ios-fluid-p0a-a4d2-acceptance.md`](ios-fluid-p0a-a4d2-acceptance.md)。
- A5：[`ios-fluid-p0a-a5-acceptance.md`](ios-fluid-p0a-a5-acceptance.md)。
- A6：[`ios-fluid-p0a-a6-acceptance.md`](ios-fluid-p0a-a6-acceptance.md)。
