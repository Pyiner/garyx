...
extension GaryxMobileModel {
    ...
    /// The first row schedules a flush; rows arriving within the interval are
    /// absorbed (the flush reads the latest window), so a catch-up burst folds into
    /// one run-state update, render, and persist. The final row always lands in a
    /// flush because the last scheduled flush reads the latest in-memory window.
