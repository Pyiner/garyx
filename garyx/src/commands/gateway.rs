...
use crate::gateway_runtime::{
    RebuildDiscovererFactory, rebuild_channel_plugins, rebuild_channel_plugins_with_factory,
};

#[cfg(test)]
pub(crate) fn routing_rebuild_channels(config: &GaryxConfig) -> Vec<String> {
    crate::gateway_runtime::routing_rebuild_channels(config)
}
...
pub(crate) async fn run_gateway(
    ...
    crate::gateway_runtime::run(config_path, port_override, host_override, no_channels).await
}
