...
impl StreamingCallbackShared {
    ...
    async fn flush_pending_stream_text(self: &Arc<Self>) {
        ...
        if self
            .roll_stream_segment_if_needed(&mut state, &content_text)
            .await
        {
            return;
        }
    ...
    async fn send_markdown_images_from_state(&self, state: &mut StreamState) {
    ...
    async fn process_boundary(&self, state: &mut StreamState) {
        let pending_boundary_text = state.stream_text.accumulated_text().to_owned();
        let _ = self
            .roll_stream_segment_if_needed(state, &pending_boundary_text)
            .await;
        ...
        if boundary_text.is_empty() {
            self.delete_runtime_only_message(state).await;
            self.send_markdown_images_from_state(state).await;
            Self::reset_for_fresh_message(state);
            return;
        }
    ...
    async fn process_tool_use(self: &Arc<Self>, state: &mut StreamState, message: ProviderMessage) {
        if !state.stream_text.accumulated_text().trim().is_empty() {
            let accumulated_text = state.stream_text.accumulated_text().to_owned();
            if self
                .roll_stream_segment_if_needed(state, &accumulated_text)
                .await
            {
    ...
    async fn process_event(self: &Arc<Self>, event: StreamEvent) {
        let mut state = self.state.lock().await;
        ...
        let is_final = match event {
            ...
            StreamEvent::Boundary { kind, .. } => match kind {
                StreamBoundaryKind::UserAck => {
                    self.process_boundary(&mut state).await;
                    return;
                }
                ...
                match decision {
                    ...
                    PluginStreamSendDecision::ScheduleFlush { after } => {
                        ...
                        tokio::spawn(async move {
                            ...
                            shared.flush_pending_stream_text().await;
                        });
                        return;
                    }
            ...
            StreamEvent::ToolUse { message } => {
                ...
                self.process_tool_use(&mut state, message).await;
                return;
            }
            StreamEvent::ToolResult { message } => {
                self.process_image_generation_result(&mut state, message)
                    .await;
                return;
            }
        ...
        if is_final && state.stream_text.is_tool_placeholder_active() {
            ...
            self.send_markdown_images_from_state(&mut state).await;
            return;
        }
...
pub(super) fn build_response_callback(
    ...
    tokio::spawn(async move {
        while let Some(event) = event_rx.recv().await {
            shared_for_worker.process_event(event).await;
        }
    });
