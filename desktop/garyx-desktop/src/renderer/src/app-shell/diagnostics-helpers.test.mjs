...
test('side-tools min width is the policy knob: legacy pinned, expand-v1 widened', () => {
  assert.equal(sideToolsPanelMinWidth('legacy'), 320);
  assert.equal(sideToolsPanelMinWidth('expand-v1'), 590);
  assert.equal(SIDE_TOOLS_PANEL_MAX_WIDTH, 1180);
});

test('defaults side tools to the policy min width regardless of canvas', () => {
  assert.equal(defaultSideToolsPanelWidth('expand-v1', 1800), 590);
  assert.equal(defaultSideToolsPanelWidth('expand-v1', 1235), 590);
  assert.equal(defaultSideToolsPanelWidth('expand-v1', 900), 590);
...
test('clamps a customized side-tools rail to the measured canvas', () => {
  assert.equal(clampSideToolsPanelWidth('expand-v1', 840, 736), 590);
...
test('side-tools width helpers match their complete boundary table', () => {
  const cases = [
    ['expand tools invalid uses default', 'expand-v1', Number.NaN, null, 590],
    ['expand tools below min', 'expand-v1', 589, null, 590],
    ['expand tools rounds', 'expand-v1', 700.5, null, 701],
    ['expand tools above max', 'expand-v1', 1181, null, 1180],
    ['expand tools narrow canvas', 'expand-v1', 840, 736, 590],
