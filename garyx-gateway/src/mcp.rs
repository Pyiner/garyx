...
#[derive(Debug, Clone, Deserialize, JsonSchema)]
#[serde(untagged)]
pub enum McpPrincipalInput {
    Principal(McpPrincipal),
    String(String),
}

#[derive(Debug, Clone, Deserialize, JsonSchema)]
pub struct TaskCreateItemParams {
    #[serde(default)]
...
pub struct TaskListParams {
    ...
    pub assignee: Option<McpPrincipalInput>,
    #[serde(default)]
    pub creator: Option<McpPrincipalInput>,
...
impl GaryMcpServer {
    ...
    #[tool(
        description = "Create one or more task threads in a channel/account scope. Requires current MCP thread context so the actor can be inferred."
    )]
    async fn task_create(
        ...
        tools::tasks::create(self, ctx, params).await
    }

    #[tool(
        description = "Promote an existing thread into a task. Requires current MCP thread context so the actor can be inferred."
    )]
    async fn task_promote(
        ...
        tools::tasks::promote(self, ctx, params).await
    }

    #[tool(
        description = "Get a task by task_ref or thread_id. If omitted, falls back to the current MCP thread."
    )]
    async fn task_get(
        &self,
        ctx: RequestContext<RoleServer>,
        ...
        tools::tasks::get(self, ctx, params).await
    }

    #[tool(description = "List tasks in a channel/account scope.")]
    async fn task_list(
        &self,
        Parameters(params): Parameters<TaskListParams>,
    ) -> Result<String, String> {
        tools::tasks::list(self, params).await
    }

    #[tool(
        description = "Read task event history by task_ref or thread_id. If omitted, falls back to the current MCP thread."
    )]
    async fn task_history(
        ...
        tools::tasks::history(self, ctx, params).await
    }

    #[tool(
        description = "Assign a task to a human or agent principal. Requires current MCP thread context so the actor can be inferred."
    )]
    async fn task_assign(
        ...
        tools::tasks::assign(self, ctx, params).await
    }

    #[tool(
        description = "Clear a task assignee. Requires current MCP thread context so the actor can be inferred."
    )]
    async fn task_unassign(
        &self,
        ctx: RequestContext<RoleServer>,
        Parameters(params): Parameters<TaskGetParams>,
    ) -> Result<String, String> {
        tools::tasks::unassign(self, ctx, params).await
    }

    #[tool(
        description = "Move a task along the task status state machine. Requires current MCP thread context so the actor can be inferred."
    )]
    async fn task_update_status(
        &self,
        ctx: RequestContext<RoleServer>,
        Parameters(params): Parameters<TaskUpdateStatusParams>,
    ) -> Result<String, String> {
        tools::tasks::update_status(self, ctx, params).await
    }

    #[tool(
        description = "Set a task title. Requires current MCP thread context so the actor can be inferred."
    )]
    async fn task_set_title(
        &self,
        ctx: RequestContext<RoleServer>,
        Parameters(params): Parameters<TaskSetTitleParams>,
    ) -> Result<String, String> {
        tools::tasks::set_title(self, ctx, params).await
    }
