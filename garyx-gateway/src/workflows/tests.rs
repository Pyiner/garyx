...
fn embedded_workflow_bun_extracts_xz_payload() {
    ...
    let bun = entrypoint::ensure_embedded_workflow_bun_extracted(
        temp.path(),
        FAKE_BUN_XZ,
        "test-version",
        "test-target",
    )
    .expect("extract embedded bun");

    assert_eq!(
        std::fs::read_to_string(&bun).expect("extracted fake bun"),
        "#!/bin/sh\necho fake-bun\n"
    );
    assert!(bun.ends_with("runtimes/bun/test-version/test-target/bun"));
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        assert_ne!(
...
#[tokio::test]
async fn task_workflow_runs_route_returns_runs_children_and_events() {
    let state = workflow_test_state().await;
    state
        .ops
        .garyx_db
        .create_workflow_run(WorkflowRunDraft {
            workflow_id: Some("task-drilldown".to_owned()),
            task_id: Some("#TASK-9".to_owned()),
            task_thread_id: Some("thread::task-9".to_owned()),
            workflow_definition_id: Some("unit".to_owned()),
            workflow_definition_version: Some(2),
            workflow_definition_snapshot_json: Some(
                r#"{"workflowId":"unit","version":2}"#.to_owned(),
            ),
            input_json: Some(json!("inspect task").to_string()),
            parent_thread_id: "thread::task-9".to_owned(),
            parent_run_id: None,
            name: "Task drilldown".to_owned(),
            description: None,
            status: "running".to_owned(),
    ...
    state
        ...
        .garyx_db
        .upsert_workflow_child_run(WorkflowChildRunDraft {
            workflow_id: "task-drilldown".to_owned(),
            workflow_child_run_id: Some("workflow-child::drilldown".to_owned()),
            thread_id: "thread::child".to_owned(),
            phase_index: 0,
            phase_title: "Inspect".to_owned(),
            label: "Inspect child".to_owned(),
            agent_id: Some("claude".to_owned()),
            status: "succeeded".to_owned(),
            prompt: "Inspect".to_owned(),
            result_mode: "text".to_owned(),
            schema_json: None,
            result_text: Some("ok".to_owned()),
            result_json: None,
            result_preview: Some("ok".to_owned()),
            error: None,
            input_tokens: 1,
            output_tokens: 2,
            tool_calls: 0,
            ...
            finished_at: Some("2026-05-29T01:00:02.000Z".to_owned()),
        })
        .expect("child");
    state
        .ops
        ...
        .append_workflow_event(WorkflowEventDraft {
            event_id: Some("workflow-event::drilldown".to_owned()),
            workflow_id: "task-drilldown".to_owned(),
            workflow_child_run_id: Some("workflow-child::drilldown".to_owned()),
    ...
    state
        ...
        .garyx_db
        .create_workflow_run(WorkflowRunDraft {
            workflow_id: Some("task-drilldown-second".to_owned()),
            task_id: Some("#TASK-9".to_owned()),
            task_thread_id: Some("thread::task-9".to_owned()),
            workflow_definition_id: Some("unit".to_owned()),
            workflow_definition_version: Some(2),
            ...
            parent_thread_id: "thread::task-9".to_owned(),
            parent_run_id: None,
            name: "Task drilldown second".to_owned(),
            description: None,
            status: "running".to_owned(),
            current_phase_index: Some(0),
            script_text: "sdk".to_owned(),
    ...
    state
        ...
        .create_workflow_run(WorkflowRunDraft {
            ...
            workflow_definition_snapshot_json: None,
            input_json: None,
            parent_thread_id: "thread::other-task".to_owned(),
            parent_run_id: None,
            ...
            workspace_dir: None,
            created_by: Some("test".to_owned()),
            started_at: Some("2026-05-29T01:00:04.000Z".to_owned()),
            finished_at: None,
        })
        .expect("other task workflow");
    ...
    let response = router
        ...
        .oneshot(
            crate::test_support::authed_request()
                .method("GET")
                .uri("/api/tasks/%23TASK-9/workflow-runs?limit=1")
                .body(Body::empty())
                .expect("request"),
        )
        .await
    ...
    let payload: Value =
        serde_json::from_slice(&to_bytes(response.into_body(), usize::MAX).await.unwrap())
            .expect("json");
    assert_eq!(payload["taskId"], "#TASK-9");
    assert_eq!(payload["count"], 1);
    assert_eq!(payload["hasMore"], true);
    assert_ne!(
        payload["workflowRuns"][0]["workflow"]["workflowRunId"],
        "other-task"
    );
    ...
    let response = router
        .oneshot(
            crate::test_support::authed_request()
                .method("GET")
                .uri("/api/tasks/%23TASK-9/workflow-runs?limit=10")
                .body(Body::empty())
                .expect("request"),
        )
        .await
    ...
    let runs = payload["workflowRuns"].as_array().expect("runs");
    assert!(
        runs.iter()
            .all(|run| run["workflow"]["workflowRunId"] != "other-task")
    );
    let drilldown = runs
        .iter()
        .find(|run| run["workflow"]["workflowRunId"] == "task-drilldown")
        .expect("task drilldown run");
    ...
    assert_eq!(
        drilldown["children"][0]["workflowChildRunId"],
        "workflow-child::drilldown"
    );
    assert_eq!(
        drilldown["events"][0]["eventType"],
        "workflow.child_succeeded"
    );
}
