...
#[tokio::test]
async fn test_route_and_dispatch_goal_command_sets_goal_and_enables_loop() {
    let store = Arc::new(InMemoryThreadStore::new());
    ...
    extra_metadata.insert(
        NATIVE_COMMAND_TEXT_METADATA_KEY.to_owned(),
        Value::String("/goal Finish the native provider".to_owned()),
    );
    extra_metadata.insert("chat_id".to_owned(), Value::String("1000000002".to_owned()));

    let request = InboundRequest {
        channel: "telegram".to_owned(),
        account_id: "bot1".to_owned(),
        from_id: "1000000002".to_owned(),
        is_group: false,
        thread_binding_key: "1000000002".to_owned(),
        message: "/goal Finish the native provider".to_owned(),
        run_id: "run-cmd-goal".to_owned(),
        reply_to_message_id: None,
        images: vec![],
        extra_metadata,
        file_paths: vec![],
    ...
    assert_eq!(
        ...
        Some("Goal set. Loop mode enabled.")
    );
    let data = store
        .get(&result.thread_id)
        .await
        .expect("thread should exist");
    assert_eq!(data["loop_enabled"], true);
    assert_eq!(data["goal"]["status"], "active");
    assert_eq!(
        data["metadata"]["goal"]["objective"],
        "Finish the native provider"
    );
    let dispatched = dispatcher.dispatched.lock().await;
    assert!(dispatched.is_empty());
}
...
#[tokio::test]
async fn test_route_and_dispatch_goal_clear_disables_loop() {
    let store = Arc::new(InMemoryThreadStore::new());
    let mut router = MessageRouter::new(store.clone(), GaryxConfig::default());
    let dispatcher = MockDispatcher::new();
    ...
    let set_result = router
        .route_and_dispatch(
            InboundRequest {
                channel: "telegram".to_owned(),
                account_id: "bot1".to_owned(),
                from_id: "1000000003".to_owned(),
                is_group: false,
                thread_binding_key: "1000000003".to_owned(),
                message: "/goal Finish the native provider".to_owned(),
                run_id: "run-cmd-goal-set".to_owned(),
                reply_to_message_id: None,
                images: vec![],
                extra_metadata: set_meta,
                file_paths: vec![],
            },
            &dispatcher,
            None,
    ...
    let mut clear_meta = HashMap::new();
    clear_meta.insert(
        NATIVE_COMMAND_TEXT_METADATA_KEY.to_owned(),
        Value::String("/goal clear".to_owned()),
    );
    let clear_result = router
        .route_and_dispatch(
            InboundRequest {
                ...
                is_group: false,
                thread_binding_key: "1000000003".to_owned(),
                message: "/goal clear".to_owned(),
                run_id: "run-cmd-goal-clear".to_owned(),
                reply_to_message_id: None,
                images: vec![],
                extra_metadata: clear_meta,
                file_paths: vec![],
            },
            &dispatcher,
            None,
    ...
    assert_eq!(clear_result.thread_id, set_result.thread_id);
    assert_eq!(
        clear_result.local_reply.as_deref(),
        Some("Goal cleared. Loop mode disabled.")
    );
    let data = store
        .get(&clear_result.thread_id)
        .await
        .expect("thread should exist");
    assert_eq!(data["loop_enabled"], false);
    assert!(data.get("goal").is_none());
    assert!(data["metadata"].get("goal").is_none());
}
