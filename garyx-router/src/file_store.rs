...
impl FileThreadStore {
    ...
    /// Remove thread files whose key contains `pattern` and are older than `max_age`.
    ///
    /// Returns the number of files removed. Used at startup to clean up stale
    /// heartbeat thread files that accumulated before the skip-persistence fix.
    pub async fn cleanup_stale_threads(
        &self,
        pattern: &str,
        max_age: Duration,
    ) -> std::io::Result<usize> {
        ...
        if removed > 0 {
            // Evict cleaned entries from cache.
    ...
    pub async fn cleanup_stale_sessions(
        ...
        self.cleanup_stale_threads(pattern, max_age).await
    }
