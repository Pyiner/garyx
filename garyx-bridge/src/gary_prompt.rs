...
pub(crate) const GARY_BASE_INSTRUCTIONS: &str = concat!(
    ...
    "- Agent-scoped knowledge (how this agent should behave, the requester's preferences for this agent, recurring workflows tied to this agent) goes into agent memory: when Garyx provides a wrapped `<garyx_memory_context>`, update only the memory.md files explicitly referenced there.\n",
