...
/// Verify that live Claude streaming input can be accepted while Claude is
/// executing a tool. The provider should acknowledge the input immediately but
/// defer writing it to Claude until the matching tool_result arrives.
#[tokio::test]
#[ignore]
async fn test_claude_streaming_input_during_tool_live() {
    if !binary_available("claude").await {
        eprintln!("claude not found, skipping");
        return;
    }
    ...
    let thread_id = "test::claude::streaming_during_tool".to_owned();
    let options = ProviderRunOptions {
        thread_id: thread_id.clone(),
        message: "Use the Bash tool exactly once to run: python3 -c 'import time; print(\"tool-start\", flush=True); time.sleep(5); print(\"tool-done\")'. Do not answer before the Bash result.".to_owned(),
        workspace_dir: None,
        images: None,
        metadata: HashMap::new(),
    };

    let saw_tool_use = Arc::new(std::sync::atomic::AtomicBool::new(false));
    let saw_tool_use_cb = saw_tool_use.clone();
    let callback: Box<dyn Fn(StreamEvent) + Send + Sync> = Box::new(move |event| {
        if matches!(event, StreamEvent::ToolUse { .. }) {
            saw_tool_use_cb.store(true, std::sync::atomic::Ordering::SeqCst);
        }
    });
    ...
    let result = tokio::time::timeout(std::time::Duration::from_secs(120), async {
        ...
        while !accepted {
            tokio::select! {
                result = &mut run_future => {
                    panic!(
                        "Claude run completed before tool-time add_streaming_input was accepted: {:?}",
                        result
                    );
                }
                _ = tokio::time::sleep(std::time::Duration::from_millis(200)) => {
                    if saw_tool_use.load(std::sync::atomic::Ordering::SeqCst) {
                        attempts += 1;
                        accepted = provider
                            .add_streaming_input(
                                &thread_id,
                                QueuedUserInput::text(
                                    "After the Bash result, reply with exactly TOOL_BOUNDARY_FOLLOW_UP_OK and no other text.",
                                )
                                .with_pending_input_id("live-tool-follow-up-1"),
                            )
                            .await;
                        if attempts >= 30 {
                            panic!("tool-time add_streaming_input was not accepted within 6s");
                        }
                    }
                }
            }
    ...
    .await
    .expect("streaming tool follow-up run timed out after 120s")
    .expect("run_streaming failed");

    eprintln!(
        "[test] streaming tool follow-up done: success={} response={}",
        result.success, result.response
    );
    assert!(
        saw_tool_use.load(std::sync::atomic::Ordering::SeqCst),
        "Expected Claude to emit a tool_use event"
    );
    assert!(
        result.success,
        "Expected successful streaming run with tool-time follow-up: {:?}",
        result.error
    );
    assert!(
        result.response.contains("TOOL_BOUNDARY_FOLLOW_UP_OK"),
        "Expected tool follow-up response marker in final response, got: {}",
        result.response
    );
}
